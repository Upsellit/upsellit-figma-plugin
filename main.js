...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getAssetThemeSnapshot = void 0;
var export_1 = require("./export");
Object.defineProperty(exports, "getAssetThemeSnapshot", { enumerable: true, get: function () { return export_1.getAssetThemeSnapshot; } });
},
...
async function buildAssetComponentNode(componentId) {
    ...
    switch (componentId) {
        ...
        case 'headline_block':
            ...
            await applyThemeText(node, theme);
        ...
        case 'eyebrow_block':
            ...
            await applyThemeText(node, theme, { charactersVariable: theme.incentive });
            applyComponentMeta(node, componentId);
            break;
        case 'divider':
        ...
        case 'product_title':
            ...
            await applyThemeText(node, theme, { colorVariable: theme.background1 });
            applyComponentMeta(node, componentId);
            break;
...
function findAssetLibraryPage() {
    for (let index = 0; index < figma.root.children.length; index += 1) {
        ...
        if (isPageNode(child) && child.name === ASSET_LIBRARY_PAGE_NAME)
            return child;
    }
    return undefined;
}
function findAssetSourceComponentNode(componentId) {
    ...
    while (stack.length) {
        ...
        if (!current)
            continue;
        const meta = getPluginMeta(current);
...
async function applyThemeToExistingComponent(node, theme) {
    ...
    if (componentId === 'optin_component') {
        ...
        if ('children' in node && Array.isArray(node.children) && node.children[0]) {
            bindUniformRadius(node.children[0], theme.borderRadius);
            bindColorVariable(node.children[0], 'strokes', theme.fontColor);
        }
    }
    ...
    if (componentId === 'progress_bar') {
        ...
        if ('children' in node && Array.isArray(node.children) && node.children[0]) {
            bindUniformRadius(node.children[0], theme.borderRadius);
            bindColorVariable(node.children[0], 'fills', theme.highlight);
        }
    }
    if (componentId === 'headline_block' || componentId === 'subtext_block' || componentId === 'disclaimer_text') {
        await applyThemeText(node, theme, { colorVariable: theme.fontColor });
    }
...
function findPageByName(name) {
    for (let index = 0; index < figma.root.children.length; index += 1) {
        const child = figma.root.children[index];
        if (isPageNode(child) && child.name === name)
            return child;
    }
    return undefined;
}
...
function applyNodeStyleToShape(node, style) {
    if (style.background)
        node.fills = [makeSolidFill(cssColorToHex(style.background))];
...
async function createNodeFromManifest(node, parentBounds, parentLayoutMode) {
    ...
    if (node.type === 'TEXT') {
        ...
        if (node.style.color)
            text.fills = [makeSolidFill(cssColorToHex(node.style.color))];
    ...
    if (node.componentOverride) {
        ...
        setPluginMeta(sceneNode, node.metadata);
    }
...
async function ensureAssetSourcePage() {
    const libraryTheme = template_library_1.BUNDLED_TEMPLATE_LIBRARY.assetTheme.length
        ...
        : template_library_1.BUNDLED_TEMPLATE_LIBRARY.entries.length
    ...
    for (let index = 0; index < constants_1.COMMON_COMPONENTS.length; index += 1) {
        ...
        if (findAssetSourceComponentNode(component.id))
            continue;
        ...
        const node = await buildAssetComponentNode(component.id);
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.createAssetComponentInstance = void 0;
var export_1 = require("./export");
Object.defineProperty(exports, "createAssetComponentInstance", { enumerable: true, get: function () { return export_1.createAssetComponentInstance; } });
},
"figma/import-library": function(require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ensureTemplatesPageFromLibrary = exports.ensureAssetSourcePage = void 0;
var export_1 = require("./export");
Object.defineProperty(exports, "ensureAssetSourcePage", { enumerable: true, get: function () { return export_1.ensureAssetSourcePage; } });
Object.defineProperty(exports, "ensureTemplatesPageFromLibrary", { enumerable: true, get: function () { return export_1.ensureTemplatesPageFromLibrary; } });
},
...
function collectClientComponentInstances(root, sourceFrameName, sourceFolder) {
    return (0, index_1.flattenTree)(root)
        .filter(function (node) {
        return !node.ignored && !!node.componentOverride && !!constants_1.COMPONENT_BY_ID[node.componentOverride];
    })
        .map(function (node) {
        const text = (node.text || '').trim();
        return {
            componentId: node.componentOverride,
...
function buildClientComponentCatalog(instances) {
    const entries = {};
    for (let index = 0; index < instances.length; index += 1) {
        const instance = instances[index];
        const definition = constants_1.COMPONENT_BY_ID[instance.componentId];
        if (!definition)
            continue;
...
async function buildExportFilesForNode(rootNode, filePrefix) {
    ...
    const raw = (0, index_3.renderRawFallback)(analysis.ast);
    ...
    const flattenedTextVariant = (0, index_3.renderFlattenedHtml)(analysis.ast, analysis, flattenedTextAssetName, true);
    const flattenedLiveVariant = (0, index_3.renderFlattenedHtml)(analysis.ast, analysis, flattenedLiveAssetName, false);
    const pageVariants = [];
    ...
    const images = [];
    if (mockupAsset)
        images.push({ name: mockupAsset.name, href: mockupAsset.name });
    if (flattenedLiveAsset)
        images.push({ name: flattenedLiveAsset.name, href: flattenedLiveAsset.name });
    if (flattenedTextAsset)
        images.push({ name: flattenedTextAsset.name, href: flattenedTextAsset.name });
    ...
    const devModeHtml = (0, index_3.renderDevModePage)(previewTitle, flattenedTextAssetName, formattedDevCss, formattedDevJs);
    ...
    const files = [
        ...
        { name: prefixed('css/fallback.css'), text: raw.css },
        {
            ...
            text: (0, string_1.formatJson)({
                schema: analysis.schema,
                report: analysis.report,
                ast: analysis.ast,
                roleMap: analysis.roleMap,
                dynamicNodeIds: analysis.dynamicNodeIds,
                commonComponents: constants_1.COMMON_COMPONENTS,
            }),
        },
        {
...
async function buildSemanticExport(rootNodes) {
        ...
        return {
            ...
            files: [
                ...
                },
                {
                    ...
                    text: (0, string_1.formatFileText)('client_components.html', (0, index_3.renderClientComponentsPage)(clientCatalog, 'Client Components')),
                },
                {
                    name: 'client_components.json',
                    text: (0, string_1.formatJson)({ components: clientCatalog }),
                },
                {
    ...
    allFiles.unshift({
        name: 'index.html',
        text: (0, string_1.formatFileText)('index.html', (0, index_3.renderMultiExportIndex)(exportEntries, 'client_components.html')),
    });
    allFiles.unshift({
        name: 'client_components.html',
        text: (0, string_1.formatFileText)('client_components.html', (0, index_3.renderClientComponentsPage)(clientCatalog, 'Client Components')),
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.renderSemanticHtml = exports.renderRawFallback = void 0;
var flattened_1 = require("./flattened");
Object.defineProperty(exports, "renderRawFallback", { enumerable: true, get: function () { return flattened_1.renderRawFallback; } });
Object.defineProperty(exports, "renderSemanticHtml", { enumerable: true, get: function () { return flattened_1.renderSemanticHtml; } });
},
...
exports.renderClientComponentsPage = renderClientComponentsPage;
exports.renderDevModePage = renderDevModePage;
...
function shouldRenderAsFlattenedHtml(definition, hideVisibleText) {
    if (!definition)
        return false;
    if (!hideVisibleText)
        return definition.role !== 'disclaimer';
...
function regionRule(selector, root, bounds) {
    ...
    return selector + ' { ' + (0, css_1.cssDeclarations)({
        width: (0, css_1.toPercent)(bounds.width, root.bounds.width),
        'min-height': (0, css_1.toPercent)(bounds.height, root.bounds.height),
    }) + ' }\n';
}
...
function renderSemanticHtml(schema, ast) {
    ...
    const closeStyle = closeNode
        ? inlineStyleAttr({
            left: (0, css_1.toPercent)(closeNode.bounds.x - ast.bounds.x, ast.bounds.width),
            top: (0, css_1.toPercent)(closeNode.bounds.y - ast.bounds.y, ast.bounds.height),
            width: (0, css_1.toPercent)(closeNode.bounds.width, ast.bounds.width),
            height: (0, css_1.toPercent)(closeNode.bounds.height, ast.bounds.height),
        })
        : '';
    ...
    const semanticComponentCss = [
        ...
        schema.disclaimer
            ? '.usi_disclaimer {\n\ttext-align: center;\n}\n'
            : '',
        (0, index_1.findNodesByRole)(ast, 'divider', 0).length
            ? '.usi_divider {\n\tdisplay:block;\n\twidth:100%;\n\theight:4px;\n\tborder:0;\n\tbackground:#1f1f1f;\n\tborder-radius:999px;\n}\n'
            : '',
...
function renderPreviewIndex(title, images) {
    const previews = [
    ...
    const galleryHtml = images.length
        ? '<section class="usi_preview_gallery"><h2>Images</h2><div class="usi_preview_gallery_grid">' +
            ...
            '</div></section>'
        : '';
    return (0, string_1.formatHtml)('<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>' +
        ...
        previews
            ...
            return '<a href="' + preview.href + '">' + (0, string_1.escapeHtml)(preview.name) + '</a>';
        })
            .join('') +
        '</div></section>' +
        galleryHtml +
        ...
        previews
            ...
            .map(function (preview) {
            return '<article class="usi_preview_panel"><h2>' + (0, string_1.escapeHtml)(preview.name) + '</h2><div class="usi_preview_frame"><iframe loading="lazy" src="' + preview.href + '" title="' + (0, string_1.escapeHtml)(preview.name) + '"></iframe></div></article>';
        })
            .join('') +
        '</section></main></body></html>');
}
function renderMultiExportIndex(entries, clientComponentsHref) {
        ...
        entries
            ...
            const galleryHtml = entry.images.length
                ? '<div class="usi_export_gallery">' +
                    ...
                    '</div>'
                : '';
            return '<article class="usi_export_card"><h2>' + (0, string_1.escapeHtml)(entry.name) + '</h2><p><a class="usi_export_open" href="' + entry.href + '">Open preview</a></p>' + galleryHtml + '<div class="usi_export_frame"><iframe loading="lazy" src="' + entry.href + '" title="' + (0, string_1.escapeHtml)(entry.name) + '"></iframe></div></article>';
        })
            .join('') +
        '</section></main></body></html>');
}
function formatLayoutSummary(entry) {
...
function renderStyleToken(label, value) {
    if (value == null || value === '')
        return '';
    return '<li><strong>' + (0, string_1.escapeHtml)(label) + ':</strong> ' + (0, string_1.escapeHtml)(String(value)) + '</li>';
}
function renderClientComponentsPage(components, title) {
    const grouped = components.reduce(function (map, entry) {
    ...
    const body = categories
        ...
        return ('<section class="usi_client_components_section">' +
            '<h2>' + (0, string_1.escapeHtml)(category.charAt(0).toUpperCase() + category.slice(1)) + '</h2>' +
            ...
            grouped[category]
                ...
                const sampleText = entry.defaultInstance.text
                    ? '<p class="usi_client_component_sample"><strong>Default text:</strong> ' + (0, string_1.escapeHtml)(entry.defaultInstance.text) + '</p>'
                    : '';
                return ('<article class="usi_client_component_card">' +
                    '<header class="usi_client_component_header">' +
                    '<div><h3>' + (0, string_1.escapeHtml)(entry.label) + '</h3><p class="usi_client_component_meta">' + (0, string_1.escapeHtml)(entry.id) + ' · ' + (0, string_1.escapeHtml)(entry.role) + '</p></div>' +
                    '<span class="usi_client_component_badge">' + entry.templateCount + ' templates</span>' +
                    '</header>' +
                    '<p class="usi_client_component_description">' + (0, string_1.escapeHtml)(entry.description) + '</p>' +
                    sampleText +
                    '<p class="usi_client_component_meta"><strong>HTML:</strong> ' + (0, string_1.escapeHtml)(entry.render.htmlTag) + ' · .' + (0, string_1.escapeHtml)(entry.render.className) + '</p>' +
                    '<p class="usi_client_component_meta"><strong>Flattened:</strong> live_text=' + (0, string_1.escapeHtml)(String(entry.render.flattened.liveText)) + ', text_baked=' + (0, string_1.escapeHtml)(String(entry.render.flattened.textBaked)) + '</p>' +
                    '<p class="usi_client_component_meta"><strong>Default source:</strong> ' + (0, string_1.escapeHtml)(entry.defaultInstance.sourceFrameName) + '</p>' +
                    '<p class="usi_client_component_meta">' + (0, string_1.escapeHtml)(formatLayoutSummary(entry)) + '</p>' +
                    ...
                    entry.templates
                        ...
                        return '<li>' + (0, string_1.escapeHtml)(templateName) + '</li>';
                    })
                        .join('') +
                    '</ul></details>' +
                    '</article>');
...
function renderDevModePage(title, bakedImageHref, cssSource, jsSource) {
    ...
    const shell = (0, string_1.formatHtml)('<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>' +
        ...
        (0, string_1.escapeHtml)(bakedImageHref) +
        '" alt="' +
    ...
    return shell
        .replace(cssPlaceholder, (0, string_1.escapeHtml)(cssSource))
        .replace(jsPlaceholder, (0, string_1.escapeHtml)(jsSource));
}
...
function renderFlattenedHtml(root, analysis, imageFileName, hideVisibleText) {
    ...
    const previewContentHtml = (closeNode ? '<button type="button" id="usi_close" aria-label="Close">×</button>' : '') +
        ...
        (showCtaInVariant ? '<button class="usi_primary_cta usi_submitbutton" onclick="usi_js.click_cta();" type="button" aria-label="' + (0, string_1.escapeHtml)(ctaLabel) + '">' + ctaInnerHtml + '</button>' : '') +
        ...
        previewSummaryHtml +
        '</article>';
    const runtimeContentHtml = (closeNode ? '<button type="button" id="usi_close" aria-label="Close">×</button>' : '') +
        '<article class="usi_flattened_semantic">' +
        '<section class="usi_main">' +
        (eyebrowText ? '<p class="' + eyebrowClass + '">' + (0, string_1.escapeHtml)(eyebrowText) + '</p>' : '') +
        (headlineText ? '<h1 class="' + headlineClass + '">' + (0, string_1.escapeHtml)(headlineText) + '</h1>' : '') +
    ...
    const html = '<!doctype html><html lang="en"><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>Legacy flattened export</title><link rel="stylesheet" href="css/' +
        ...
        (0, string_1.escapeHtml)(headlineText || 'Preview') +
        ...
        (0, string_1.escapeHtml)(imageFileName) +
    ...
    const js = buildPriceRuntimeSetup(hasSummary) +
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.renderPreviewIndex = exports.renderMultiExportIndex = exports.renderMockupReviewIndex = exports.renderClientComponentsPage = void 0;
var flattened_1 = require("./flattened");
Object.defineProperty(exports, "renderClientComponentsPage", { enumerable: true, get: function () { return flattened_1.renderClientComponentsPage; } });
Object.defineProperty(exports, "renderMockupReviewIndex", { enumerable: true, get: function () { return flattened_1.renderMockupReviewIndex; } });
Object.defineProperty(exports, "renderMultiExportIndex", { enumerable: true, get: function () { return flattened_1.renderMultiExportIndex; } });
Object.defineProperty(exports, "renderPreviewIndex", { enumerable: true, get: function () { return flattened_1.renderPreviewIndex; } });
},
"render/devmode": function(require, module, exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.renderDevModePage = exports.extractCampaignCss = void 0;
var flattened_1 = require("./flattened");
Object.defineProperty(exports, "extractCampaignCss", { enumerable: true, get: function () { return flattened_1.extractCampaignCss; } });
Object.defineProperty(exports, "renderDevModePage", { enumerable: true, get: function () { return flattened_1.renderDevModePage; } });
}
};
var cache = {};
